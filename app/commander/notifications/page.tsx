...
export default function CommanderNotificationsPage() {
  ...
  async function markAllRead() {
    ...
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) return;
    await fetch("/api/commander/notifications", {
      method: "PATCH",
    ...
    <div>
      ...
      {items.length === 0 ? (
        ...
        <div className="grid gap-2">
          {items.map((n) => (
            <div key={n.id}
              className={`card flex items-start gap-3 ${n.is_read ? "opacity-60" : "border-nebula-purple/40"}`}>
              <span className="mt-0.5 text-2xl">{TYPE_ICON[n.type] ?? "🔔"}</span>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  {!n.is_read && <span className="h-2 w-2 shrink-0 rounded-full bg-nebula-pink" />}
